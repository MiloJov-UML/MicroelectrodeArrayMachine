...
def return_to_origin(axis=None):
    ...
    if motor_ser:
        if axis:
            command = f"H{axis}1\r"  # Command for returning a specific axis to origin
            response = send_command(motor_ser, command, "Motor Controller")
            ...
            command = "HX0 HY0 HZ0 Hr0 Ht0 HT0\r"  # Command for returning all axes to origin
            response = send_command(motor_ser, command, "Motor Controller")
            if response and "ERR" in response:
...
def run_full_manual_loop():
    ...
    try:
        ...
        move_linear_stage("t", "+", 570)  
        time.sleep(1)
        move_linear_stage("t", "-", 550)  
        time.sleep(1)
        move_linear_stage("X", "-", 500)  
        time.sleep(2)
        move_linear_stage("t", "+", 570)  
        time.sleep(1)
        move_linear_stage("t", "-", 550)  
